...
update_custom() {
    ...
    printf "%s\n" "${new_lines[@]}" > "$tmp_file"
    # Eliminar duplicados en el archivo resultante (se conserva la primera ocurrencia de cada línea)
    awk '!seen[$0]++' "$tmp_file" > "${tmp_file}_uniq"
...
for CUSTOM_FILE in "${CUSTOM_FILES[@]}"; do
    if [ ! -f "$CUSTOM_FILE" ]; then
        ...
        tmpfile=$(mktemp)
        awk '!seen[$0]++' "$CUSTOM_FILE" > "$tmpfile"
