...
update_custom() {
    ...
    # Procesar cada línea hasta el máximo de líneas
    for (( i=0; i<max; i++ )); do
        local main_line=""
        local custom_line=""
        if [ $i -lt $n_main ]; then
    ...
    printf "%s\n" "${new_lines[@]}" > "$tmp_file"
    # Eliminar duplicados en el archivo resultante, permitiendo duplicados en líneas vacías o que empiecen con "#"
    awk '{
        if ($0 ~ /^[[:space:]]*$/ || $0 ~ /^[[:space:]]*#/) {
            print
        } else if (!($0 in seen)) {
...
for CUSTOM_FILE in "${CUSTOM_FILES[@]}"; do
    if [ ! -f "$CUSTOM_FILE" ]; then
        ...
        # Eliminar duplicados con excepción de líneas en blanco y comentadas
        tmpfile=$(mktemp)
        awk '{
            if ($0 ~ /^[[:space:]]*$/ || $0 ~ /^[[:space:]]*#/) {
                print
